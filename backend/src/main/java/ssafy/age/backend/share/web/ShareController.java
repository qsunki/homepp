...
public class ShareController {
    ...
    public List<ShareDto> getAllShares(@AuthenticationPrincipal MemberInfoDto memberInfoDto) {
        return shareService.getAllShares(memberInfoDto.email());
    }

    @Operation(summary = "비디오 공유해 줄 회원 추가", description = "로그인 된 회원의 비디오를 공유해 줄 회원(이메일과 닉네임)을 추가합니다.")
    @PostMapping
    public ShareDto createShare(@AuthenticationPrincipal MemberInfoDto memberInfoDto, @RequestBody ShareDto shareDto) {
        return shareService.createShare(memberInfoDto.email(), shareDto.email(), shareDto.nickname());
    }
    ...
    public ShareDto updateShare(
        ...
        return shareService.updateShare(memberInfoDto.email(), sharedMemberEmail, shareDto.nickname());
    }

    @Operation(summary = "비디오 공유해 줄 회원 삭제", description = "공유 받았던 회원의 이메일로 비디오를 공유할 목록에서 삭제합니다.")
    @DeleteMapping("/{sharedMemberEmail}")
    public void deleteShare(
            @AuthenticationPrincipal MemberInfoDto memberInfoDto, @PathVariable String sharedMemberEmail) {
        shareService.deleteShare(memberInfoDto.email(), sharedMemberEmail);
    }
}
